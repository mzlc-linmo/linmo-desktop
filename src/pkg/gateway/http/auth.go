...
const (
	...
	envGatewayToken     = "LIMNO_GATEWAY_TOKEN"
)
...
// Config takes precedence; env LIMNO_GATEWAY_TOKEN overrides when set.
...
func writeTokenError(w http.ResponseWriter) {
	...
	_, _ = w.Write([]byte(`{"error":"网关令牌无效或未提供。请在 Control UI 的 Overview 中配置正确的 Gateway Token，或使用 linmo doctor --generate-gateway-token 生成。","code":"invalid_gateway_token"}`))
}
