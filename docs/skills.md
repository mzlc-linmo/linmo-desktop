...
Linmo 的 Skills 系统基于 [`agentsdk-go` 的 Skills 抽象](https://docs.linmo.ai/zh-CN/tools/skills) 扩展而来，用于为大模型会话注入领域知识、工具和工作流能力。Skills 通常以 `SKILL.md` 文件形式存在，支持从多个位置加载与合并。
...
2. **内置 Skills**：安装包自带的 `skills` 目录（`LIMNO_BUNDLED_SKILLS_DIR` 或可执行文件旁的 `skills/`）（低优先级）
3. **托管 Skills**：`~/.linmo/skills`（中等优先级）
...
3. 构造 `Entry`：
   - `Name`：最终 Skill 名称
   - `Source`：来源标记，如 `linmo-workspace`、`linmo-managed` 等
...
- Runtime 会根据用户输入、上下文、Skill 的元数据等决定是否激活某个 Skill（匹配逻辑在 agentsdk-go 内部，遵循 [`skills` 文档](https://docs.linmo.ai/zh-CN/tools/skills) 中的通用规则）。
...
- **团队共享通过 extraDirs/managedSkills**：将公共 Skill 放到 `~/.linmo/skills` 或配置的 `extraDirs`，在多个项目间复用。
