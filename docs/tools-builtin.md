...
Linmo 通过 `pkg/agent/runtime/builtin_tools.go` 将 [`agentsdk-go` 的内置工具](https://github.com/stellarlinkco/agentsdk-go/tree/main/docs)注入到运行时中，为大模型提供一套开箱即用的通用能力。
...
Linmo 在创建 Runtime 时通过 `buildSandboxOptions` 传入默认的：
