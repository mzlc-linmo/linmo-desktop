...
import (
	...
	"github.com/openocta/openocta/cmd/linmo/commands"
...
func main() {
	...
	if err := config.LoadEnvFromCurrentDir(); err != nil {
		fmt.Fprintf(os.Stderr, "linmo: warning: load .env: %v\n", err)
	}
	if err := config.EnsureDefaultConfig(config.DefaultEnv); err != nil {
		fmt.Fprintf(os.Stderr, "linmo: init config: %v\n", err)
		os.Exit(1)
	}
	// Deploy: if ~/.linmo/workspace/prompt has no .md files, copy from cwd/prompt
	stateDir := paths.ResolveStateDir(config.DefaultEnv)
	profile := strings.TrimSpace(config.DefaultEnv("LIMNO_PROFILE"))
	...
	if err := runtime.EnsureWorkspacePrompts(workspaceDir, promptSourceDir); err != nil {
		fmt.Fprintf(os.Stderr, "linmo: warning: ensure workspace prompts: %v\n", err)
	}
	commands.Execute()
}
